...
The web app (`http://localhost:5000`) and the API (`http://localhost:5001`) are **different origins**. `apps/api/src/main.ts` never called `app.enableCors()`, so the browser blocked every request (preflight `OPTIONS /api/v1/auth/login` returned **404**, no `Access-Control-*` headers). `curl` worked because CORS is browser-enforced only. The login page's `catch {}` turned *any* error — including this network/CORS failure — into the message **"Invalid credentials"**, so a correct password still showed that text.
...
- **`apps/api/src/config/env.validation.ts` + `.env(.example)`** — new validated `CORS_ORIGIN` (default `http://localhost:5000`).
...
| 8 | ▲ CORS preflight `OPTIONS /api/v1/auth/login` (Origin :5000) → 2xx + `Access-Control-Allow-Origin: http://localhost:5000` | e2e + live curl | **Passed** |
...
> **Update 2026-06-15:** dev ports moved to **web `:5000` / api `:5001`** (was `:3000/:3001`). The `:3000/:3001` references above are historical; see [`dev-ports-5000-5001.md`](dev-ports-5000-5001.md).
