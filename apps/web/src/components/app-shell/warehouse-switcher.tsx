...
export function WarehouseSwitcher() {
  ...
  // TODO(phase-6): lift selectedId into a shared warehouse context when the real scope switcher lands.
  ...
  if (isLoading) {
    return (
      <button
        type="button"
        disabled
        className="flex h-[38px] items-center gap-2 rounded-[10px] border border-border bg-surface-2 px-3 text-left opacity-60"
        aria-label="Loading warehouse"
      >
        ...
        <span className="leading-tight">
          ...
          <span className="block text-[12.5px] font-semibold">—</span>
        </span>
