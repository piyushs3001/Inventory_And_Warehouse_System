...
export function RequestTransferDialog({
  ...
  const [sourceWarehouseId, setSourceWarehouseId] = useState(warehouses.length === 1 ? warehouses[0].id : '');
  ...
  const onSubmit = async (e: FormEvent): Promise<void> => {
    ...
    try {
      await create.mutateAsync({
        data: { sourceWarehouseId, destinationWarehouseId, notes: notes || undefined, lines: parsed },
      });
      await onSuccess();
      onClose();
    } catch (err) {
